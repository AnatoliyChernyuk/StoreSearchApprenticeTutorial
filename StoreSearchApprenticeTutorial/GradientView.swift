...
//
//  Created by Anatoliy Chernyuk on 12/3/17.
...
class GradientView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        let componennts: [CGFloat] = [0, 0, 0, 0.3, 0, 0, 0, 0.7]
        let locations: [CGFloat] = [0, 1]
        let colorSpace = CGColorSpaceCreateDeviceRGB()
        let gradient = CGGradient(colorSpace: colorSpace, colorComponents: componennts, locations: locations, count: 2)
        let x = bounds.midX
        let y = bounds.midY
